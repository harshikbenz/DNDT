...
def main():
    ...
    # For baseline model, one-hot encode labels (2 classes).
    y_train_baseline = one_hot_encode(y_train, 2)
    y_test_baseline  = one_hot_encode(y_test, 2)
    
    # -------------------------------
    # Train Baseline Model
    # -------------------------------
    print("\nTraining Baseline Model (Conventional Network)")
    baseline_model = build_baseline_model(input_shape=(X_train.shape[1],), num_classes=2)
    ...
    # ---- Iteration 1: Preliminary run to determine full convergence ----
    ...
    cascaded_model.fit(X_train, new_labels_list, epochs=50, batch_size=16, verbose=1)
